...
V tdi(I O,I d) {
  //dummy(O);
  Fi(d, printf("  "))
  //printf("%d v:%d s:%d c:%d\n", O, NP(O)->v, NP(O)->s, NP(O)->c);
  printf("%d\n", NP(O)->v);
  fflush(stdout);
  TRAV(C, O) tdi(C,d+1);
}
...
N1* run(I R, char *pr, I pl) {
  ...
  Fi(pl,
    ...
    switch(pr[i]) {
      ...
      B
      case'C':                            // Depending on r1.v set r2 to
        O = NP(C)->c;                     // - child of C if r1.v = 0
        Fi(NP(r1)->v,                     // - sibling of C otherwise
          O = NP(C)->s
        );
        r2=O;
      B
      case'D':                            // Depending on r1.v assign r2 to
        O = NP(C)->c;                     // - sibling of child of C if r1.v = 1
        Fi(NP(r1)->v - 1,                 // - sibling of sibling of C otherwise
          O = NP(C)->s
        );
        NP(O)->s = r2;
      B
